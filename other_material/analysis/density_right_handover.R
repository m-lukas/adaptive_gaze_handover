# File: density_within_trial_right_handover_location.R
# Plots density of within-trial timing for target = right_handover_location
...
for (i in seq_len(nrow(meta_pairs))) {
    ...
    dat <- g_tr %>%
        filter(target == "right_handover_location") %>%
...
if (nrow(face_intervals_all) == 0) stop("No data for right_handover_location.")
...
# Create density plot for right-handover

dir.create("figures", showWarnings = FALSE)

p_density_right <- face_intervals_all %>%
    ...
    ylab("Density") +
    ggtitle("Within-trial timing: right_handover_location")

ggsave("figures/density_within_trial_right_handover_location.png",
       p_density_right, width = 10, height = 7, dpi = 200)
