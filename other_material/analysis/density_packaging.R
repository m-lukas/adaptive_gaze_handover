suppressPackageStartupMessages({
    library(dplyr); library(purrr); library(readr); library(tidyr); library(tidyverse); library(lubridate); library(janitor); library(glue); library(data.table)
})
...
if (nrow(face_intervals_all) == 0) stop("No data for packaging_area.")
...
# Create density plot for packaging

dir.create("figures", showWarnings = FALSE)

p_density_packaging <- face_intervals_all %>%
    ...
    ylab("Density") +
    ggtitle("Within-trial timing: packaging_area")

ggsave("figures/density_within_trial_packaging_area.png",
       p_density_packaging, width = 10, height = 7, dpi = 200)
...
density_minima <- face_intervals_all %>%
    ...
    group_map(.f = function(df, key) {
        v <- df$rel_mid_s
        if (length(v) < 5) return(tibble())  # not enough points
...
if (!nrow(density_minima)) {
    ...
    dir.create("figures", showWarnings = FALSE)
    write_csv(density_minima, "figures/packaging_density_local_minima.csv")
}
